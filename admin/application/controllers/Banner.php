<?php
    class Banner extends MY_Controller{
        ...
        public function index(){
		    ...
		    $config['total_rows'] = $this->appmodel->num_rows("banners", $where, $filters);
		    $config['per_page'] = 10;
		    ...
		    $items = $this->appmodel->select_all("banners", $where, $filters);
		...
		public function create(){
			...
			if($post_data){
    			...
    			$config['image_library'] = 'gd2';
					$config['source_image'] = './assets/images/banner/'.$image;
				...
				$isInsert = $this->appmodel->insert_one('banners', $insert_data);
				if($isInsert){
		...
		public function edit($id){
			...
			if($post_data){
				...
				$isInsert = $this->appmodel->update('banners', $where, $insert_data);
				if($isInsert){
		    ...
		    $item = $this->appmodel->select_one("banners", $where);
		    $data = [
		    	'item'   				=> $item  
		    ];
		...
		public function delete($id){
			...
			$isDelete = $this->appmodel->delete_one("banners", $where);
			if($isDelete){
		...
		public function delete_all(){
			$isDelete = $this->appmodel->delete_all("banners");
			if($isDelete){
		...
		public function dublicate($id){
		    ...
		    if($dublicate){
				...
				$isDublicate = $this->appmodel->insert_one('banners', $insert);
				if($isDublicate){
		...
		public function export(){
			...
			$usersData = $this->appmodel->select_all('banners', $where, $filters);
			...
			if($post){
				...
				while(($filesop = fgetcsv($handle, 1000, ",")) !== false) {
					if($c<>0){ /* SKIP THE FIRST ROW */
						...
						if(!$this->appmodel->select_one('banners', $where)){
							$this->appmodel->insert_one('banners', $update);
						}else{
							$this->appmodel->update('banners', $where, $update);
						}
						$isInsert = true;
					}
					$c = $c + 1;
