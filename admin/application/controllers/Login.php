...
class Login extends Ci_controller{
    ...
    public function signin(){
        ...
        if($this->form_validation->run()){
            $usr = $this->mymodel->select_one('*', ' (email="'.$formdata['user'].'" or phone="'.$formdata['user'].'")', 'vapp_admin');        
    ...
    private function send_mail($msg, $to, $subject){
        ...
        public function customer_login(){
           ...
           $this->form_validation->set_rules('password', 'password', 'required');
           if($this->form_validation->run()){
            $usr = $this->mymodel->select_one('*', ' (email="'.$formdata['user'].'" and phone="'.$formdata['password'].'")', 'customer');       
            if($usr){
                if($usr->phone === $formdata['password']){
                    $this->session->set_userdata('remember_me', $this->input->post('remember_me'));
